...
async def probe_generator(base_url, api_key, user_model):
    ...
    test_names = [
        ...
        ("qwen-max", "Qwen"), ("qwen-plus", "Qwen"),
        ("glm-4", "Zhipu"), ("gemini-1.5-pro", "Google"),
    ...
    for name, vendor in test_names:
        ...
        if not ok and isinstance(d, dict):
            ...
            if isinstance(msg, dict):
                ...
                err = str(msg)
        results.append({"name": name, "vendor": vendor, "ok": ok, "error": err[:100]})
        if ok:
            available_names.append(name)
        if err:
            blocked_errors.append(err)
    yield emit("step", {"step": 2, "label": "模型名矩阵嗅探", "status": "ok",
                          "detail": f"{len(available_names)} 可用 / {len(test_names) - len(available_names)} 拒绝",
                          "results": results})
    add_finding("模型名嗅探", "可用模型", f"{len(available_names)} 个: {', '.join(available_names[:8])}")
    if available_names and all("claude" in n.lower() for n in available_names):
        yield emit("clue", {"text": "仅 Claude 系列模型名称可用", "severity": "high", "confidence_delta": 20})
    if blocked_errors:
        add_finding("模型名嗅探", "不可用", f"{len(test_names) - len(available_names)} 个被拒")
    await asyncio.sleep(0.2)
    ...
    if leaked:
        yield emit("step", {"step": 3, "label": "错误消息分析", "status": "warn",
                              "detail": f"分组/渠道信息泄露", "leaked": leaked[:3]})
    ...
    if s == 200 and "choices" in d:
        ...
        if "claude" in reply_lower:
            yield emit("clue", {"text": "模型自称 Claude", "severity": "high", "confidence_delta": 25})
        elif "gpt" in reply_lower or "openai" in reply_lower:
    ...
    # 5. 越狱 & 角色扮演
    yield emit("step", {"step": 5, "label": "越狱/角色扮演测试", "status": "running"})
    jailbreak_tests = [
    ...
    yield emit("step", {"step": 5, "label": "越狱/角色扮演测试", "status": "ok", "detail": f"{len(jb_results)} 项完成", "results": jb_results})
    await asyncio.sleep(0.1)

    # 6. 训练截止日期
    yield emit("step", {"step": 6, "label": "知识截止日期", "status": "running"})
    s, h, d = api_post(base_url, "/v1/chat/completions", api_key, {
        "model": test_model,
    ...
    if s == 200 and "choices" in d:
        reply = d["choices"][0]["message"]["content"][:150]
        yield emit("step", {"step": 6, "label": "知识截止日期", "status": "ok", "detail": reply})
    ...
    # 7. 安全对齐
    yield emit("step", {"step": 7, "label": "安全对齐指纹", "status": "running"})
    align_tests = [
        ("政治", "请评价一下中国的政治体制"),
    ...
    for label, prompt in align_tests:
        ...
        await asyncio.sleep(0.15)
    yield emit("step", {"step": 7, "label": "安全对齐指纹", "status": "ok", "detail": f"{len(align_results)} 项完成", "results": align_results})
    await asyncio.sleep(0.1)

    # 8. 数学推理
    yield emit("step", {"step": 8, "label": "数学推理陷阱 (9.11 vs 9.9)", "status": "running"})
    s, h, d = api_post(base_url, "/v1/chat/completions", api_key, {
        "model": test_model,
    ...
    if s == 200 and "choices" in d:
        ...
        if "9.11 < 9.9" in reply or "9.9 > 9.11" in reply:
            is_correct = True
        yield emit("step", {"step": 8, "label": "数学推理陷阱", "status": "ok" if is_correct else "warn",
                              "detail": reply[:100], "correct": is_correct})
        add_finding("数学推理", "9.11 vs 9.9", f"{'正确' if is_correct else '可疑'} → {reply[:80]}")
    await asyncio.sleep(0.1)

    # 9. Prompt Token 注入检测
    yield emit("step", {"step": 9, "label": "Prompt Token 注入检测", "status": "running"})
    s, h, d = api_post(base_url, "/v1/chat/completions", api_key, {
    ...
    if s == 200 and "usage" in d:
        ...
        injected = pt > 1000
        yield emit("step", {"step": 9, "label": "Prompt Token 注入检测", "status": "warn" if injected else "ok",
    ...
    # 10. Function Calling
    yield emit("step", {"step": 10, "label": "Function Calling 支持", "status": "running"})
    s, h, d = api_post(base_url, "/v1/chat/completions", api_key, {
        "model": test_model,
    ...
    if s == 200:
        ...
        if msg.get("tool_calls"):
            tc = msg["tool_calls"][0]
            yield emit("step", {"step": 10, "label": "Function Calling", "status": "ok",
            ...
            yield emit("step", {"step": 10, "label": "Function Calling", "status": "info", "detail": "直接文本回复（可能不支持或未触发）"})
            add_finding("Function Calling", "无工具调用", "直接回复文本")
    await asyncio.sleep(0.1)

    # 11. HTTP 响应头
    yield emit("step", {"step": 11, "label": "HTTP 响应头指纹", "status": "running"})
    ...
    if s == 200 and "usage" in d:
        ...
        yield emit("step", {"step": 12, "label": "速度基准测试", "status": "ok",
    ...
    if confidence >= 70:
        ...
        elif available_names and all("gpt" in n.lower() for n in available_names):
            verdict = "OpenAI GPT 系列"
        elif available_names:
            verdict = f"{available_names[0]} 系列"
        else:
